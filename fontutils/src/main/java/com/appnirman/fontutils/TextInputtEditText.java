...
public class TextInputtEditText extends TextInputEditText {

    private boolean deletePressed = false;

    public TextInputtEditText(Context context) {
        super(context);
        init(null, 0);
    }

    public TextInputtEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public TextInputtEditText(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs, defStyle);
    }

    public TextInputtEditText init(AttributeSet attrs, int defStyle) {
    ...
    private class CustomInputConnection extends InputConnectionWrapper {
        ...
        public boolean sendKeyEvent(KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN && event.getKeyCode() == KeyEvent.KEYCODE_DEL)
                TextInputtEditText.this.deletePressed = true;
